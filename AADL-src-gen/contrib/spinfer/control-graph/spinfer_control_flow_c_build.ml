...
let mk_node node labels bclabels nodestr =
    (
        (
            node,
            {labels=labels; bclabels=bclabels; is_loop=false; is_fake=false}
        ),
        nodestr
   )
...
let add_bc_node node labels parent_labels nodestr g =
   g#add_node (mk_node node labels parent_labels  nodestr)
...
let then_start_str = "[then_start]"
let then_end_str = "[then_end]"
let fallthrough_start_str = "[fallthrough_start]"
let fallthrough_end_str = "[fallthrough_end]"
let else_start_str = "[else_start]"
let else_end_str = "[else_end]"
let else_node_str = "else"

let ifdef_then_start_str = "[ifdef_then_start]"
let ifdef_then_end_str = "[ifdef_then_end]"
let ifdef_fallthrough_start_str = "[ifdef_fallthrough_start]"
let ifdef_fallthrough_end_str = "[ifdef_fallthrough_end]"
let ifdef_else_start_str = "[ifdef_else_start]"
let ifdef_else_end_str = "[ifdef_else_end]"
let ifdef_else_node_str = "ifdef_else"
 ...
 fun (starti, xi) stmt ->
  ...
  | Iteration  (Ast_c.While (e, st)) ->
      ...
      *)
      ...
      !g +> add_arc_opt (starti, newi);
      let newfakebranching = !g +> add_node Fake  lbl "[whileloopbranching]" in
      let newfakestart = !g +> add_node InLoopNode lbl "[loopstart]" in
      let newfakend = !g +> add_node InLoopNode lbl "[loopend]" in
      ...
      !g#add_arc ((newi, newfakebranching), Direct);
      !g#add_arc ((newfakebranching, newfakestart), Direct);
      !g#add_arc ((newfakend,newfakebranching), Direct);
      !g#add_arc ((newafter, newfakeelse), Direct);
      !g#add_arc ((newfakebranching, newafter), Direct);
      let finalthen = aux_statement (Some newfakestart, newxi) st in
      !g +> add_arc_opt
	(finalthen, if !Flag_parsing_c.no_loops then newafter else newfakend);
      Some newfakeelse
  ...
  (* FIXME: Do while CFG-augmentation is not supported atm,
    need to patch fragment.ml *)
      ...
      *)
      ...
      in
      (*let newfakestart = !g +> add_node InLoopNode  lbl "[loopstart]" in*)
      let newfakeend = !g +> add_node InLoopNode  lbl "[loopend]" in
      ...
      (*!g#add_arc ((doi,newfakestart), Direct);
      !g#add_arc ((newfakestart,afteri), Direct);*)
      ...
      | Some finali ->
          !g#add_arc ((finali, newfakeend), Direct);
          !g#add_arc ((newfakeend, taili), Direct);
          Some newfakeelse
      )
  ...
  | Iteration  (Ast_c.For (e1opt, e2opt, e3opt, st)) ->
      ...
      !g +> add_arc_opt (starti, newi);
      let newfakebranching = !g +> add_node Fake  lbl "[loopbranching]" in
      let newfakestart = !g +> add_node InLoopNode  lbl "[loopstart]" in
      let newfakeend = !g +> add_node InLoopNode  lbl "[loopend]" in
      ...
      !g#add_arc ((newi, newfakebranching), Direct);
      !g#add_arc ((newfakebranching, newfakestart), Direct);
      !g#add_arc ((newfakeend, newfakebranching), Direct);
      !g#add_arc ((newafter, newfakeelse), Direct);
      !g#add_arc ((newfakebranching, newafter), Direct);
      let finalthen = aux_statement (Some newfakestart, newxi) st in
      !g +> add_arc_opt
	(finalthen,
	 if !Flag_parsing_c.no_loops then newafter else newfakeend);
      Some newfakeelse
  ...
  | Iteration  (Ast_c.MacroIteration (s, es, st)) ->
      ...
      !g +> add_arc_opt (starti, newi);
      let newfakebranching = !g +> add_node Fake  lbl "[macroloopbranching]" in
      let newfakestart = !g +> add_node InLoopNode  lbl "[macroloopstart]" in
      let newfakeend = !g +> add_node InLoopNode  lbl "[macroloopend]" in
  ...
  | Jump ((Ast_c.Continue|Ast_c.Break) as x) ->
    ...
    (match x with
          | Ast_c.Continue ->
          (Continue (stmt, ((), ii)), "continue;")
          | Ast_c.Break    ->
	        (Break    (stmt, ((), ii), from_switch), "break;")
  ...
  | Jump ((Ast_c.Return | Ast_c.ReturnExpr _) as kind) ->
     ...
     | Some exiti, Some errorexiti ->
      ...
      (*let newi = insert_all_braces xi.braces newfake RetAfterNode "[ret after]" in*)
...
and mk_If (starti :nodei option) (labels :int list) (xi_lbl :xinfo)
  ...
  | Selection (Ast_c.If (e, st1, st2)) ->
    ...
    | Ast_c.ExprStatement None when iist2=[] ->
      ...
      let newfakethen =
        !g +> add_node (TrueNode escapes) labels then_start_str
      in
      let newfakeelse =
        !g +> add_node FallThroughNode labels fallthrough_start_str
      in

      let newendthen =
        !g +> add_node (TrueNode escapes) labels then_end_str
      in
      let newendelse =
        !g +> add_node FallThroughNode labels fallthrough_end_str
      in
      ...
      !g#add_arc ((newfakeelse, newendelse), Direct);

      !g#add_arc ((newendelse, lasti), Direct);
      ...
      (* [then] -> {st1} -> lasti *)
      let finalthen = match Ast_c.unwrap st1 with
      | Ast_c.Compound compound ->
          aux_statement_list (Some newfakethen) (newxi, newxi) compound
      | _ -> aux_statement (Some newfakethen, newxi) st1
      in
      (match finalthen with None -> escapes := true | _ -> ());
      !g +> add_arc_opt (finalthen, newendthen);
      !g#add_arc ((newendthen, lasti), Direct);
      lasti, Some lasti
    | __else__ ->
      (* starti -> newi --->   newfakethen -> ... -> finalthen --> lasti
       *                 |                                      |
       *          elsenode->   newfakeelse -> ... -> finalelse -|
       * update: there is now also a link directly to lasti.
       *)
      ...
      let else_labels =
        if !Flag_parsing_c.label_strategy_2
        then labels
        else labels @ [!counter_for_labels]
      in
      let elsenode =
        !g +> add_node (Else iielse) else_labels else_node_str
      in
      let newfakeelse =
        !g +> add_node FalseNode else_labels else_start_str
      in
      let newendelse =
        !g +> add_node FalseNode else_labels else_end_str
      in

      !g#add_arc ((newi, newfakethen), Direct);
      !g#add_arc ((newi, elsenode), Direct);
      !g#add_arc ((elsenode, newfakeelse), Direct);
      ...
      let finalelse = aux_statement (Some newfakeelse, newxi) st2 in
      ...
      elsenode_succ, begin match finalthen, finalelse with
        ...
        | __else__ ->
          ...
          begin
            ...
            !g +> add_arc_opt (finalelse, newendelse);
            !g#add_arc ((newendelse, lasti), Direct);
...
and aux_statement_list starti (xi, newxi) statxs =
  ...
  +> List.fold_left (fun starti statement_seq ->
    ...
    let ifdef_labels =
      ...
      newxi'.labels@[!counter_for_labels]
    in
    ...
    | Ast_c.IfdefStmt2 (ifdefs, xxs) ->
        ...
        let newi =
          !g +> add_node (IfdefHeader (head)) ifdef_labels "[ifdef]" in
        let taili =
          !g +> add_node (IfdefEndif (tail)) ifdef_labels "[ifdef_endif]" in
        (* do like for a close brace, see endi.{c,cocci} *)
        let taili_dup =
          mk_fake_node (IfdefEndif (tail)) ifdef_labels [] "[ifdef_endif]" in
        let then_start =
          !g +> add_node (TrueNode (escapes)) ifdef_labels ifdef_then_start_str in
        let then_end =
          !g +> add_node (TrueNode (escapes)) ifdef_labels ifdef_then_end_str in
        !g +> add_arc_opt (starti, newi) ;
        !g#add_arc ((newi, then_start), Direct);
        !g#add_arc ((then_end, taili), Direct);

        if body = [] then
        begin
          let newfakeelse_start =
            !g +> add_node FallThroughNode ifdef_labels ifdef_fallthrough_start_str
          in
          let newfakeelse_end =
            !g +> add_node FallThroughNode ifdef_labels ifdef_fallthrough_end_str
          in
          !g#add_arc ((newi, newfakeelse_start), Direct);
          !g#add_arc ((newfakeelse_start, newfakeelse_end), Direct);
          !g#add_arc ((newfakeelse_end, taili), Direct);
        end;

        let else_labels =
          if !Flag_parsing_c.label_strategy_2
          then ifdef_labels
          else ifdef_labels @ [!counter_for_labels]
        in
        let elsenodes =
          body +> List.map (fun elseif ->
            let elsei =
              !g +> add_node (IfdefElse (elseif)) else_labels ifdef_else_node_str in
            !g#add_arc ((newi, elsei), Direct);
            elsei
          ) in

        let _finalxs =
          Common.zip (then_start::elsenodes) xxs +> List.map (fun (start_nodei, xs)->
              ...
              !g +> add_arc_opt (finalthen, then_end);
              )
        in
...
*)
  ...
  | Ast_c.CppTop (Ast_c.Pragma ((id,ii), pragmainfo))  ->
      ...
      let str = "#pragma " ^ Ast_c.str_of_name id in
      let ei =   !g +> add_node elem    lbl_0 str in
      let endi = !g +> add_node EndNode lbl_0 "[end]" in
